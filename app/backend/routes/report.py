...
from app.backend.models.report import ReportCreate, ReportResponse, ReportStatus, ReportListResponse
...
async def get_reports(
    ...
    return await report_controller.get_reports(report_status=report_status, page=page, size=size)
...
@router.put("/admin/reports/{report_id}/status", response_model=ReportResponse)
async def update_report_status(
        report_id: str,
        new_status: ReportStatus,
        _=Depends(admin_required)
):
    updated_report = await report_controller.update_report_status(report_id, new_status)
